...
namespace SfQuery
{
    public class SalesforceClient
    {
        ...
        public static System.Configuration.ConnectionStringSettingsCollection ConnectionStrings { get; set; }
        ...
        public string QueryNext(string soqlQuery)
        {
            using (var client = new HttpClient())
            {
                //string restRequest = InstanceUrl + API_ENDPOINT + "query/?q=" + soqlQuery;
                string restRequest = soqlQuery;
                var request = new HttpRequestMessage(HttpMethod.Get, restRequest);
                request.Headers.Add("Authorization", "Bearer " + AuthToken);
                request.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                request.Headers.Add("X-PrettyPrint", "1");
